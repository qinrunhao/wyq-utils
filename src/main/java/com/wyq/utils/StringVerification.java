...
public class StringVerification {
    ...
    public static String brief(String str, int len) {
        if (StringUtils.isEmpty(str) || (str.length() + 3) <= len) {
            return str;
        }
        int w = len / 2;
        int l = str.length();
    ...
    public static boolean isin(String[] ss, String s) {
        if (null == ss || ss.length == 0 || StringUtils.isEmpty(s)) {
            return false;
        }
        for (String w : ss) {
            if (s.equals(w)) {
    ...
    public static Pattern P_CitizenId = Pattern.compile("^[1-9]\\d{5}[1-9]\\d{3}((0\\d)|(1[0-2]))(([0|1|2]\\d)|3[0-1])\\d{3}([0-9]|X)$");
    public static Pattern P_Mobile = Pattern.compile("^[1][3,4,5,7,8][0-9]{9}$");
    ...
    public static Pattern P_LoginValid = Pattern.compile("^[0-9a-zA-Z]+$");
    ...
    public static String removeLast(String str, char c) {
        return (StringUtils.isEmpty(str) || !str.endsWith(c + "")) ? str
                : str.subSequence(0,
                str.length() - 1)
                .toString();
    }

    public static boolean loginValid(String str) {
        if (isBlank(str)) {
            return false;
        }
        return isMactchRegex(P_LoginValid,str);
